...
def film_details(movie_id):
    ...
    for x in db.comments.find({'movie_id': id}):
        ...
        try:
            cu = db.users.find_one({'username': current_user.username})['move_comment']
            if [x['username'], movie_id] not in cu:
                comments.append(x)
        except:
            comments.append(x)
        x['avatar'] = av['avatar']
        ...
        if not current_user.is_authenticated:
            ...
            try:
                ...
                return render_template('/details/film_details.html', avatar=avatar, username=username, gender=gender,
                                       ...
                                       director = director, film_description=film_description, issue_date=issue_date,postform=postform, comments=comments, new_rating=rate, similar_movies=similar_movies, cur_wlist=cur_wlist, id=id, movies_db=movies_db, current_user=current_user)
            except:
                return render_template('/details/film_details.html', avatar=avatar, username=username, gender=gender,
...
def delete_comments(username, movie_id):
    try:
        move_comment = db.users.find_one({"username": current_user.username})['move_comment']
        move_comment.append((username, movie_id))
        db.users.update_one({'username': current_user.username}, {"$set": {"move_comment": move_comment}})
    except:
        move_comment = [(username, movie_id)]
        db.users.update_one({'username': current_user.username}, {"$set": {"move_comment": move_comment}})
